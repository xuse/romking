...
import io.github.xuse.simple.context.util.chinese.ChineseCharProvider;


/**
 * 生成随机数据，以及由随机数据填充的复杂bean。一般用来制作演示或生成测试数据。
 * 
 * @author Joey
 *
 */
public class RandomData {
	...
	private static Object newInstance(java.lang.reflect.Type type, int level, RandomValue anno) {
		...
		if (type == Integer.class || type == Integer.TYPE) {
			...
			case AUTO:
				return prefix(randomString(ChineseCharProvider.getInstance().get(ChineseCharProvider.Type.CHINESE_LAST_NAME), minLen, len), anno);
			case NAME:
