...
public class RomFile {
	...
	/**
	 * 游戏标准名（No-Intro英文名，或从文件名推断的原始名）
	 */
	@ColumnSpec(name="name",nullable = false,size = 128,type = Types.VARCHAR,defaultValue = "''")
	private String name;
	
	/**
	 * 显示名（中文名，来自gamelist.xml或用户手动设置）。
	 * UI展示时优先使用displayName，为空则回退到name。
	 */
	@ColumnSpec(name="display_name",nullable = true,size = 128,type = Types.VARCHAR)
	...
	/**
	 * 文件状态：OK/MISSING/CORRUPTED
	 */
	@ColumnSpec(name="file_status", nullable = false, type = Types.TINYINT, unsigned = true, defaultValue = "0")
	@CustomType(EnumByCodeType.class)
	@ViewColumn(caption="文件状态")
