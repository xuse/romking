...
public class ZipInputStream extends InflaterInputStream implements ZipConstants {
	...
	public ZipInputStream(InputStream in, Charset encoding) {
	...
	/**
	 * Reads from the current ZIP entry into an array of bytes. If <code>len</code>
	...
	public int read(byte[] b, int off, int len) throws IOException {
		...
		case STORED:
			...
			if (remaining == 0 && entry.getCrc() != crc.getValue()) {
				throw new ZipException("invalid entry CRC (expected 0x" + Long.toHexString(entry.getCrc())
	...
	private void readEnd(ZipEntry e) throws IOException {
		...
		if (e.getSize() != inf.getBytesWritten()) {
			throw new ZipException(
					"invalid entry size (expected " + e.getSize() + " but got " + inf.getBytesWritten() + " bytes)");
		}
	...
	/*
	 * Fetches unsigned 16-bit value from byte array at specified offset. The bytes
	 * are assumed to be in Intel (little-endian) byte order.
	 */
	...
	/*
	 * Fetches unsigned 32-bit value from byte array at specified offset. The bytes
	 * are assumed to be in Intel (little-endian) byte order.
	 */
