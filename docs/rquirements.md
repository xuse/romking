...
| 元数据补全（跨仓库 MD5 匹配） | ✅ 已完成 | 归档时继承源仓库 displayName 等元数据 |
...
| 利用 INSTANCE 仓库元数据补全归档仓库 | ✅ 已完成 | 归档时 copyRomFile 携带 displayName |
...
| 增量扫描逻辑（跳过未变更文件） | ✅ 已完成 | 基于 size+lastModified 判断，变更时删旧记录重新入库 |
| ZIP 文件 CRC 快速检测 | ✅ 已完成 | 从 ZIP 头直接读取，零解压 |
| 大文件 MD5 延迟计算 | ✅ 已完成 | computeMd5 选项控制，首次扫描可选跳过 |
...
| TaskType.VERIFY | ✅ 已完成 | |
| 导出时跳过异常状态条目 | ✅ 已完成 | ExportRomTask 过滤 MISSING/CORRUPTED |
...
| 批量清理（保留一个，删除其余） | ✅ 已完成 | batchCleanExactDuplicates() + UI按钮 |
...
| ROM 搜索与筛选 | ✅ 已完成 | RomSearchView 支持游戏名/ROM文件名/平台筛选 |
...
| 提交导出任务 | ✅ 已完成 | |
| 任务历史展示 | ✅ 已完成 | |
| 导出模板/预设 | ✅ 已完成 | ExportPreset 实体 + 保存/加载预设 UI |
